...
<script type="text/javascript">
    let labelsetat = [
                      @foreach ($formateurs as $formateur)
                        '{{ $formateur->nomuser }} {{ $formateur->prenomuser }}',
                      @endforeach
                  ];
    let myChartetat = document.getElementById("etat").getContext('2d');

    let etat = new Chart(myChartetat, {
        type: 'radar',
        data: {
            labels: labelsetat,
            datasets: [
              {
                ...
                data: [
                           @foreach ($formateurs as $formateur)
                             '{{ $formateur->postionnements_count }}',
                           @endforeach
                  ]
              }
            ]
